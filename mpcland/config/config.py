...
# =============== 仿真基本参数 ===============
DELTA_T = 0.08  # 时间步长 (秒)。这是整个系统中离散时间仿真的基本单位。
MAX_EPISODE_TIME = 50.0  # 单次仿真的最大时长 (秒)。
...
class Quadrotor:
    ...
    OMEGA_MAX = np.array([0.6, 0.6, 0.4])  # [wx_max, wy_max, wz_max]
...
class MPC:
    """定义了MPC专家控制器的核心参数。"""
    HORIZON = 12  # 预测时域 N。MPC会向前看10个时间步来做优化。
...
class Termination:
    ...
    SUCCESS_XY_ERR_MAX = 0.40  # 成功着陆时，无人机在平台坐标系下的最大水平误差 (m)
